		...
		extends RingElementTestBase<Rational>
{
	...
	public void divide()
	{
		...
		Rational r = one.divide(rFac.m_one());
	...
	public void invert()
	{
		Rational r1 = rFac.get(-1, 1);
		assertEquals("1/-1)=-1", rFac.m_one(), r1.invert());
	...
	public void misc()
	{
		...
		Rational n = five_over_two.negate();
	...
	{
		assertEquals(rFac.one(), rFac.one().divide(rFac.one()));
		assertEquals(rFac.m_one(), rFac.m_one().divide(rFac.one()));
