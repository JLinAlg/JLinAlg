...
public class Xor
{
	...
	public Xor()
	{
		df = new LinAlgFactory<>(DoubleWrapper.FACTORY);
	...
	public void train()
	{
		...
		for (int i = 0; i < NEPOCH; ++i) {
			...
			for (int j = 0; j < npat; ++j) {
				...
				Vector<DoubleWrapper> ai = new Vector<>(inpat[j],
				...
				Vector<DoubleWrapper> eo = new Vector<>(tgpat[j],
			...
			wih = wih.add(dwih.divide(npatd).multiply(eta))
					.add(dwih1.divide(npatd).multiply(mu));
			who = who.add(dwho.divide(npatd).multiply(eta))
					.add(dwho1.divide(npatd).multiply(mu));
			bh = bh.add(dbh.divide(npatd).multiply(eta))
					.add(dbh1.divide(npatd).multiply(mu));
			bo = bo.add(dbo.divide(npatd).multiply(eta))
					.add(dbo1.divide(npatd).multiply(mu));
			...
			if (i == 0 || i == NEPOCH - 1 || ((i + 1) % 1000) == 0) {
				System.err.println("EPOCH: " + (i + 1) + "\tRMS ERROR: " + Math
						.sqrt(((sqrerr.sum()).divide(errd)).doubleValue()));
			}
		}
	}
	...
	public void test()
	{
		...
		for (int j = 0; j < npat; ++j) {
			...
			Vector<DoubleWrapper> ai = new Vector<>(inpat[j],
					DoubleWrapper.FACTORY);
			Vector<DoubleWrapper> tg = new Vector<>(tgpat[j],
					DoubleWrapper.FACTORY);
